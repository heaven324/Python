...
def cal(oprator, n, expression):
    if n == 2:
        return str(eval(expression))
    if oprator[n] == '*':
        expression = expression.split('*')
        for i in range(len(expression)):
            expression[i] = cal(oprator, n+1, expression[i])
...
def solution(expression):
    ...
    for i in range(len(op)):
        res = abs(int(cal(op[i], 0, expression)))
        if answer < res:
            answer = res
    return answer
...
IT 벤처 회사를 운영하고 있는 라이언은 매년 사내 해커톤 대회를 개최하여 우승자에게 
상금을 지급하고 있습니다.
이번 대회에서는 우승자에게 지급되는 상금을 이전 대회와는 다르게 다음과 
같은 방식으로 결정하려고 합니다.
해커톤 대회에 참가하는 모든 참가자들에게는 숫자들과 3가지의 연산문자(+, -, *) 
만으로 이루어진 연산 수식이 전달되며, 참가자의 미션은 전달받은 수식에 포함된 연산자의 
우선순위를 자유롭게 재정의하여 만들 수 있는 가장 큰 숫자를 제출하는 것입니다.
...
이루어진 올바른 중위표기법(연산의 두 대상 사이에 연산기호를 사용하는 방식)으로 표현된 연산식입니다. 
