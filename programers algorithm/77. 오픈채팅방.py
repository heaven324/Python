...
def solution(record):
    ...
    for i in range(len(record)):
        sent = record[i].split(' ')
        if sent[0] == "Enter":
            id_dic[sent[1]] = sent[2]
            ans.append([sent[0], sent[1]])
        elif sent[0] == "Change":
            id_dic[sent[1]] = sent[2]
        else:
            ans.append([sent[0], sent[1]])
...
예를 들어, 채팅방에 Muzi와 Prodo라는 닉네임을 사용하는 사람이 순서대로 
들어오면 채팅방에는 다음과 같이 메시지가 출력된다.
...
Muzi가 나간후 다시 들어올 때, 
Prodo 라는 닉네임으로 들어올 경우 기존에 채팅방에 남아있던 Muzi도 
Prodo로 다음과 같이 변경된다.
