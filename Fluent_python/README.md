...
| `s.count(e)`               | o    |       | `e`요소가 발생한 횟수를 반환한다.                                             |
| `s.__delitem__(p)`         | o    | o     | `p`위치의 항목을 삭제한다.                                                    |
| `s.extend(i)`              | o    | o     | 반복 가능한 `i`에 있는 요소를 오른쪽에 추가한다.                              |
| `s.extendleft(i)`          | o    | o     | 반복 가능한 `i`에 있는 요소를 왼쪽에 추가한다.                                |
| `s.__getitem__(p)`         | o    | o     | `s[p]` -- `p`위치의 항목을 가져온다.                                          |
| `s.index(e)`               | o    | o     | 처음 `e`가 나타난 위치를 반환한다.                                            |
| `s.insert(p, e)`           | o    | o     | `p`위치의 항목 앞에 `e`요소를 추가한다.                                       |
| `s.__iter__()`             | o    | o     | 반복자를 반환한다.                                                            |
| `s.__len__()`              | o    | o     | `len(s)` -- 항목 수를 반환한다.                                               |
| `s.__mul__(n)`             | o    | o     | `s * n` -- 반복해서 연결한다.                                                 |
| `s._imul__(n)`             | o    | o     | `s *= n` -- 반복해서 연결한 후 `s`에 저장한다.                                |
| `s.__rmul__(n)`            | o    | o     | `n * s` -- 역순 반복 연결.                                                    |
| `s.pop([p])`               | o    | o     | 마지막 항목을 삭제하고 반환한다.                                              |
| `s.popleft([p])`           | o    | o     | 첫 번째 항목을 삭제하고 반환한다.                                             |
| `s.remove(e)`              | o    | o     | `e`와 같은 값을 가진 첫 번째 항목을 삭제한다.                                 |
| `s.reverse()`              | o    | o     | 항목을 역순으로 나열해서 다시 `s`에 저장한다.                                 |
| `s.__reversed__()`         | o    |       | 뒤에서부터 앞으로 나열하는 반복자를 반환한다.                                 |
